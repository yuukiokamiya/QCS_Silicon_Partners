...
static uint8_t find_conn_idx_from_bd_addr(const st_ble_dev_addr_t *p_addr) {
	for (uint8_t i = 0; i < BLE_SERVC_MAX_NUM_OF_SAVED; i++) {
		if ((gs_conn_info[i].bd_addr.type == p_addr->type)
				&& (0
						== memcmp(gs_conn_info[i].bd_addr.addr, p_addr->addr,
								BLE_BD_ADDR_LEN))) {
			return i;
		}
	}
...
static void set_conn_idx(uint16_t conn_hdl, const st_ble_dev_addr_t *p_addr) {
	...
	for (uint8_t i = 0; i < BLE_SERVC_MAX_NUM_OF_SAVED; i++) {
		if (BLE_GAP_INVALID_CONN_HDL == gs_conn_info[i].conn_hdl) {
			memcpy(&gs_conn_info[i].bd_addr, p_addr,
					sizeof(gs_conn_info[i].bd_addr));
			gs_conn_info[i].conn_hdl = conn_hdl;
			break;
		}
	}
...
static void find_attr(uint8_t conn_idx, uint16_t attr_hdl,
	...
	for (uint8_t s = 0; s < gs_num_of_clients; s++) {
		for (uint8_t c = 0; c < gs_clients[s]->num_of_chars; c++) {
			if ((gs_clients[s]->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl
					<= attr_hdl)
					&& (gs_clients[s]->pp_chars[c]->p_attr_hdls[conn_idx].end_hdl
							>= attr_hdl)) {
...
static void read_evt_handler(uint16_t conn_hdl, uint16_t attr_hdl,
		st_ble_gatt_value_t *p_value, ble_status_t result) {
		...
		if (attr_hdl == p_attr->p_attr_hdls[conn_idx].start_hdl + 1) {
			if (BLE_SUCCESS == result) {
				...
				if (NULL != p_attr->read_rsp_cb) {
					...
					st_ble_servc_evt_data_t evt_data = { .conn_hdl = conn_hdl,
							.param_len = p_attr->app_size, .p_param =
									p_app_value, };
					p_client->cb(
							BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx,
									p_attr->inst_idx, BLE_SERVC_READ_RSP),
			...
			for (uint8_t d = 0; d < p_attr->num_of_descs; d++) {
				if (attr_hdl == p_attr->pp_descs[d]->p_attr_hdls[conn_idx]) {
					if (BLE_SUCCESS == result) {
						...
						memset(p_app_value, 0x00,
								p_attr->pp_descs[d]->app_size);

						ret = p_attr->pp_descs[d]->decode(p_app_value, p_value);

						if (NULL != p_attr->pp_descs[d]->read_rsp_cb) {
							p_attr->pp_descs[d]->read_rsp_cb(
									&p_attr->pp_descs[d], conn_hdl, ret,
									p_app_value);
						} else {
							st_ble_servc_evt_data_t evt_data = { .conn_hdl =
									conn_hdl, .param_len =
									p_attr->pp_descs[d]->app_size, .p_param =
									p_app_value, };
							p_client->cb(
									BLE_SERVC_MULTI_ATTR_EVENT(
											p_attr->pp_descs[d]->desc_idx,
											p_attr->inst_idx,
											BLE_SERVC_READ_RSP), BLE_SUCCESS,
									&evt_data);
						}
						...
						if (NULL != p_attr->pp_descs[d]->read_rsp_cb) {
							p_attr->pp_descs[d]->read_rsp_cb(
									&p_attr->pp_descs[d], conn_hdl, result,
									NULL);
						} else {
							st_ble_servc_evt_data_t evt_data = { .conn_hdl =
									conn_hdl, .param_len = 0, .p_param = NULL, };
							p_client->cb(
									BLE_SERVC_MULTI_ATTR_EVENT(
											p_attr->pp_descs[d]->desc_idx,
											p_attr->inst_idx,
											BLE_SERVC_READ_RSP), result,
									&evt_data);
						}
					}
					break;
...
static void write_evt_handler(uint16_t conn_hdl, uint16_t attr_hdl,
		ble_status_t result) {
	uint8_t conn_idx = find_conn_idx_from_conn_hdl(conn_hdl);
	...
	if (NULL != p_attr) {
		...
		if (attr_hdl == p_attr->p_attr_hdls[conn_idx].start_hdl + 1) {
			if (NULL != p_attr->write_rsp_cb) {
				...
				p_client->cb(
						BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx,
								p_attr->inst_idx, BLE_SERVC_WRITE_RSP), result,
						&evt_data);
			}
		}
		/* Descriptors */
			...
			for (uint8_t d = 0; d < p_attr->num_of_descs; d++) {
				if (attr_hdl == p_attr->pp_descs[d]->p_attr_hdls[conn_idx]) {
					if (NULL != p_attr->pp_descs[d]->write_rsp_cb) {
						p_attr->pp_descs[d]->write_rsp_cb(&p_attr->pp_descs[d],
								conn_hdl, result);
					} else {
						st_ble_servc_evt_data_t evt_data = { .conn_hdl =
								conn_hdl, .param_len = 0, .p_param = NULL, };
						p_client->cb(
								BLE_SERVC_MULTI_ATTR_EVENT(
										p_attr->pp_descs[d]->desc_idx,
										p_attr->inst_idx, BLE_SERVC_WRITE_RSP),
								result, &evt_data);
					}
					break;
				}
...
void R_BLE_SERVC_GattcCb(uint16_t type, ble_status_t result,
	...
	case BLE_GATTC_EVENT_CHAR_READ_RSP: {
		...
		read_evt_handler(p_data->conn_hdl,
				...
				&p_rd_char_evt_param->read_data.value, result);
	}
		break;
	...
	case BLE_GATTC_EVENT_CHAR_PART_READ_RSP: {
		...
		const st_ble_servc_info_t *p_client = NULL;
		const st_ble_servc_char_info_t *p_attr = NULL;
		find_attr(conn_idx, p_rd_char_evt_param->read_data.attr_hdl, &p_client,
				&p_attr);
			...
			if (p_rd_char_evt_param->read_data.attr_hdl
				...
				if (BLE_SUCCESS == result) {
					if (NULL == gs_conn_info[conn_idx].p_read_buf) {
						gs_conn_info[conn_idx].p_read_buf = malloc(
								p_attr->db_size);
						memset(gs_conn_info[conn_idx].p_read_buf, 0x00,
								p_attr->db_size);
					...
					memcpy(
							&gs_conn_info[conn_idx].p_read_buf[gs_conn_info[conn_idx].read_buf_pos],
					...
					if (NULL != p_attr->read_rsp_cb) {
						p_attr->read_rsp_cb(p_attr, p_data->conn_hdl, result,
								NULL);
					} else {
						st_ble_servc_evt_data_t evt_data = { .conn_hdl =
								p_data->conn_hdl, .param_len = 0, .p_param =
								NULL, };
						p_client->cb(
								BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx,
										p_attr->inst_idx, BLE_SERVC_READ_RSP),
				...
				for (uint8_t d = 0; d < p_attr->num_of_descs; d++) {
					if (p_rd_char_evt_param->read_data.attr_hdl
						...
						if (BLE_SUCCESS == result) {
							...
							if (NULL != p_attr->pp_descs[d]->read_rsp_cb) {
								p_attr->pp_descs[d]->read_rsp_cb(p_attr,
										p_data->conn_hdl, result, NULL);
							} else {
								st_ble_servc_evt_data_t evt_data = { .conn_hdl =
										p_data->conn_hdl, .param_len = 0,
										.p_param = NULL, };
								p_client->cb(
										BLE_SERVC_MULTI_ATTR_EVENT(
												p_attr->pp_descs[d]->desc_idx,
	...
	case BLE_GATTC_EVENT_LONG_CHAR_READ_COMP: {
		...
		st_ble_gatt_value_t value = { .p_value =
				gs_conn_info[conn_idx].p_read_buf, .value_len =
				gs_conn_info[conn_idx].read_buf_pos, };

		read_evt_handler(p_data->conn_hdl, gs_conn_info[conn_idx].read_attr_hdl,
				&value, result);
	...
	case BLE_GATTC_EVENT_CHAR_WRITE_RSP: {
		...
		write_evt_handler(p_data->conn_hdl, p_wr_char_evt_param->value_hdl,
				result);
	}
		break;
	...
	case BLE_GATTC_EVENT_CHAR_PART_WRITE_RSP: {
		...
		gs_conn_info[conn_idx].write_attr_hdl =
				p_char_part_wr_param->write_data.attr_hdl;
	}
		break;

	case BLE_GATTC_EVENT_LONG_CHAR_WRITE_COMP: {
		uint8_t conn_idx = find_conn_idx_from_conn_hdl(p_data->conn_hdl);
		write_evt_handler(p_data->conn_hdl,
				gs_conn_info[conn_idx].write_attr_hdl, result);
	}
		break;
	...
	case BLE_GATTC_EVENT_HDL_VAL_NTF: {
		...
		const st_ble_servc_info_t *p_client = NULL;
		const st_ble_servc_char_info_t *p_attr = NULL;
		find_attr(conn_idx, p_hdl_value_pair_param->attr_hdl, &p_client,
				&p_attr);
		...
		if ((NULL != p_attr) && (NULL != p_attr->decode)) {
			...
			if (NULL != p_attr->hdl_val_ntf_cb) {
				...
				st_ble_servc_evt_data_t evt_data = { .conn_hdl =
						p_data->conn_hdl, .param_len = p_attr->app_size,
						.p_param = p_app_value, };
				p_client->cb(
						BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx,
								p_attr->inst_idx, BLE_SERVC_HDL_VAL_NTF),
						BLE_SUCCESS, &evt_data);
			}
	...
	case BLE_GATTC_EVENT_HDL_VAL_IND: {
		...
		if ((NULL != p_attr) && (NULL != p_attr->decode)) {
			...
			if (NULL != p_attr->hdl_val_ind_cb) {
				...
				p_client->cb(
						BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx,
								p_attr->inst_idx, BLE_SERVC_HDL_VAL_IND),
						BLE_SUCCESS, &evt_data);
			}
	...
	case BLE_GATTC_EVENT_ERROR_RSP: {
		...
		if (NULL != p_attr) {
			...
			{
				if (NULL != p_attr->read_rsp_cb) {
					p_attr->read_rsp_cb(p_attr, p_data->conn_hdl,
							p_err_rsp_evt_param->rsp_code, NULL);
				} else {
					st_ble_servc_evt_data_t evt_data = { .conn_hdl =
							p_data->conn_hdl, .param_len = 2, .p_param =
							&p_err_rsp_evt_param->attr_hdl, };
					p_client->cb(
							BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx,
									p_attr->inst_idx, BLE_SERVC_READ_RSP),
							p_err_rsp_evt_param->rsp_code, &evt_data);
				}
			}
				break;
			...
			{
				if (NULL != p_attr->write_rsp_cb) {
					p_attr->write_rsp_cb(p_attr, p_data->conn_hdl,
...
ble_status_t R_BLE_SERVC_ReadChar(const st_ble_servc_char_info_t *p_attr,
	...
	if (p_attr->db_size <= (mtu - 3)) {
		ret = R_BLE_GATTC_ReadChar(conn_hdl,
				(uint16_t)(p_attr->p_attr_hdls[conn_idx].start_hdl + 1));
	} else {
		ret = R_BLE_GATTC_ReadLongChar(conn_hdl,
				(uint16_t)(p_attr->p_attr_hdls[conn_idx].start_hdl + 1), 0);
	}
...
ble_status_t R_BLE_SERVC_WriteChar(const st_ble_servc_char_info_t *p_attr,
		uint16_t conn_hdl, const void *p_app_value) {
	ble_status_t ret;
	...
	st_ble_gatt_value_t gatt_value = { .p_value = p_byte_value, .value_len =
			p_attr->db_size, };

	ret = p_attr->encode(p_app_value, &gatt_value);

	st_ble_gatt_hdl_value_pair_t write_data = { .attr_hdl = (uint16_t)(
			p_attr->p_attr_hdls[conn_idx].start_hdl + 1), .value.p_value =
			gatt_value.p_value, .value.value_len = gatt_value.value_len, };
...
ble_status_t R_BLE_SERVC_ReadDesc(const st_ble_servc_desc_info_t *p_attr,
	...
	if (p_attr->db_size <= (mtu - 1)) {
		...
		return R_BLE_GATTC_ReadLongChar(conn_hdl, p_attr->p_attr_hdls[conn_idx],
				0);
	}
}

ble_status_t R_BLE_SERVC_ReadDesc_with_Type(
		const st_ble_servc_desc_info_t *p_attr, uint16_t conn_hdl,
		int32_t type) {
	if (NULL == p_attr) {
		return BLE_ERR_INVALID_ARG;
	}
...
ble_status_t R_BLE_SERVC_WriteDesc(const st_ble_servc_desc_info_t *p_attr,
	...
	st_ble_gatt_hdl_value_pair_t write_data = { .attr_hdl =
			p_attr->p_attr_hdls[conn_idx], .value.p_value = gatt_value.p_value,
...
ble_status_t R_BLE_SERVC_WriteDesc_with_Size(
	...
	uint16_t write_len;
	write_len =
			(p_app_value->len < p_attr->db_size) ?
					p_app_value->len : p_attr->db_size;
...
void R_BLE_SERVC_ServDiscCb(const st_ble_servc_info_t *p_info,
		uint16_t conn_hdl, uint8_t serv_idx, uint16_t type, void *p_param) {
	uint8_t conn_idx = find_conn_idx_from_conn_hdl(conn_hdl);
	/* unused arg */
	(void) serv_idx;
	...
	case BLE_DISC_PRIM_SERV_FOUND: {
		...
		memcpy(&p_info->p_attr_hdls[conn_idx],
				&p_serv_param->value.serv_16.range,
	...
	case BLE_DISC_CHAR_FOUND: {
		...
		for (uint8_t c = 0; c < p_info->num_of_chars; c++) {
			if ((BLE_GATT_16_BIT_UUID_FORMAT == p_char_param->uuid_type)
					&& (p_info->pp_chars[c]->uuid_type
							== p_char_param->uuid_type)
					&& (p_info->pp_chars[c]->uuid_16
							== p_char_param->value.char_16.uuid_16)
					&& (BLE_GATT_INVALID_ATTR_HDL_VAL
							== p_info->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl)) {
				p_info->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl =
						p_char_param->value.char_16.decl_hdl;
				...
				for (uint8_t d = 0; d < p_char_param->num_of_descs; d++) {
					for (uint8_t dd = 0; dd < p_info->pp_chars[c]->num_of_descs;
							dd++) {
						if ((p_info->pp_chars[c]->pp_descs[dd]->uuid_16
							...
							break;
								...
								== memcmp(
										p_info->pp_chars[c]->pp_descs[dd]->uuid_128,
										p_char_param->descs[d].value.desc_128.uuid_128,
										0x10))
								&& (BLE_GATT_INVALID_ATTR_HDL_VAL
										== p_info->pp_chars[c]->pp_descs[dd]->p_attr_hdls[conn_idx])) {
							p_info->pp_chars[c]->pp_descs[dd]->p_attr_hdls[conn_idx] =
				...
				break;
					...
					&& (p_info->pp_chars[c]->uuid_type
							== p_char_param->uuid_type)
					&& (0
							== memcmp(p_info->pp_chars[c]->uuid_128,
									p_char_param->value.char_128.uuid_128, 0x10))
					&& (BLE_GATT_INVALID_ATTR_HDL_VAL
							== p_info->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl)) {
				p_info->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl =
						p_char_param->value.char_128.decl_hdl;
				...
				if (p_char_param->num_of_descs > 0) {
					...
					p_info->pp_chars[c]->p_attr_hdls[conn_idx].end_hdl =
							p_char_param->value.char_128.value_hdl;
				}
