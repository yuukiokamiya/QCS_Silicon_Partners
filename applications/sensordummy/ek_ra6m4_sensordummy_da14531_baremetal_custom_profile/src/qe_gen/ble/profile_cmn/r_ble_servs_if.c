...
static void find_attr(uint16_t attr_hdl, const st_ble_servs_info_t **pp_serv,
...
static void desc_write_evt_handler(uint16_t conn_hdl, uint16_t attr_hdl,
		...
		if (NULL != p_char->pp_descs) {
			for (uint8_t d = 0; d < p_char->num_of_descs; d++) {
				if ((p_char->pp_descs[d]->attr_hdl == attr_hdl)
					...
					ret = p_char->pp_descs[d]->decode(p_app_value,
							(const st_ble_gatt_value_t*) p_value);

					if (NULL != p_char->pp_descs[d]->write_req_cb) {
						p_char->pp_descs[d]->write_req_cb(&p_char->pp_descs[d],
								conn_hdl, ret, (const void*) p_app_value);
					} else {
						st_ble_servs_evt_data_t evt_data = { .conn_hdl =
								conn_hdl, .param_len =
								p_char->pp_descs[d]->app_size, .p_param =
								p_app_value, };
						p_serv->cb(
								BLE_SERVS_MULTI_ATTR_EVENT(
										p_char->pp_descs[d]->desc_idx,
										p_char->pp_descs[d]->inst_idx,
										BLE_SERVS_WRITE_REQ), BLE_SUCCESS,
								&evt_data);
					}
			...
			for (uint8_t d = 0; d < p_char->num_of_descs; d++) {
				if ((p_char->pp_descs[d]->attr_hdl == attr_hdl)
					...
					if (NULL != p_char->pp_descs[d]->read_req_cb) {
						p_char->pp_descs[d]->read_req_cb(&p_char->pp_descs[d],
								conn_hdl);
					} else {
						st_ble_servs_evt_data_t evt_data = { .conn_hdl =
						...
						p_serv->cb(
								BLE_SERVS_MULTI_ATTR_EVENT(
										p_char->pp_descs[d]->desc_idx,
										p_char->pp_descs[d]->inst_idx,
										BLE_SERVS_READ_REQ), BLE_SUCCESS,
								&evt_data);
					}
...
static void ble_gatts_db_app_cb(uint16_t conn_hdl, uint8_t db_op,
		uint16_t attr_hdl, st_ble_gatt_value_t *p_value) {
	const st_ble_servs_info_t *p_serv = NULL;
	const st_ble_servs_char_info_t *p_char = NULL;
	...
	case BLE_GATTS_OP_CHAR_PEER_READ_REQ: {
		if (NULL != p_char->read_req_cb) {
			...
			st_ble_servs_evt_data_t evt_data = { .conn_hdl = conn_hdl,
					.param_len = 0, .p_param = NULL, };
			p_serv->cb(
					BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx,
							p_char->inst_idx, BLE_SERVS_READ_REQ), BLE_SUCCESS,
					&evt_data);
		}
	}
		break;
	...
	case BLE_GATTS_OP_CHAR_PEER_WRITE_REQ: {
		if (NULL != p_char->decode) {
			...
			if (NULL != p_char->write_req_cb) {
				...
				st_ble_servs_evt_data_t evt_data = { .conn_hdl = conn_hdl,
						.param_len = p_char->app_size, .p_param = p_app_value, };

				p_serv->cb(
						BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx,
								p_char->inst_idx, BLE_SERVS_WRITE_REQ), ret,
						&evt_data);
			}
	...
	case BLE_GATTS_OP_CHAR_PEER_WRITE_CMD: {
		if (NULL != p_char->decode) {
			...
			if (NULL != p_char->write_cmd_cb) {
				...
				p_serv->cb(
						BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx,
								p_char->inst_idx, BLE_SERVS_WRITE_CMD), ret,
						&evt_data);
			}
...
void R_BLE_SERVS_GattsCb(uint16_t type, ble_status_t result,
		...
		if ((NULL != p_serv) && (NULL != p_char)) {
			if ((p_char->start_hdl + 1) == p_cfm_evt_param->attr_hdl) {
				if (NULL != p_char->hdl_val_cnf_cb) {
					...
					st_ble_servs_evt_data_t evt_data = { .conn_hdl =
							p_data->conn_hdl, .param_len = 0, .p_param = NULL, };
					p_serv->cb(
							BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx,
									p_char->inst_idx, BLE_SERVS_HDL_VAL_CNF),
							result, &evt_data);
				}
			}
		}
	...
	case BLE_GATTS_EVENT_WRITE_RSP_COMP: {
		...
		if ((NULL != p_serv) && (NULL != p_char)) {
			/* Characteristic */
			if (((p_char->start_hdl + 1) == p_write_rsp_evt_param->attr_hdl)
					&& (NULL != p_char->decode)) {
				ble_status_t ret;
				...
				ret = R_BLE_SERVS_GetChar(p_char, p_data->conn_hdl,
						p_app_value);

				if (NULL != p_char->write_comp_cb) {
					p_char->write_comp_cb(p_char, p_data->conn_hdl, result,
							p_app_value);
				} else {
					st_ble_servs_evt_data_t evt_data = { .conn_hdl =
							p_data->conn_hdl, .param_len = p_char->app_size,
							.p_param = p_app_value, };
					p_serv->cb(
							BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx,
									p_char->inst_idx, BLE_SERVS_WRITE_COMP),
							ret, &evt_data);
				}
			...
			/* Descriptor */
				...
				if (NULL != p_char->pp_descs) {
					for (uint8_t d = 0; d < p_char->num_of_descs; d++) {
						if ((p_char->pp_descs[d]->attr_hdl
								== p_write_rsp_evt_param->attr_hdl)
								&& (NULL != p_char->pp_descs[d]->decode)) {
							ble_status_t ret;
							...
							st_ble_gatt_value_t gatt_value;
							R_BLE_GATTS_GetAttr(p_data->conn_hdl,
									p_write_rsp_evt_param->attr_hdl,
									&gatt_value);
							...
							if (NULL != p_char->pp_descs[d]->write_comp_cb) {
								p_char->pp_descs[d]->write_comp_cb(
										&p_char->pp_descs[d], p_data->conn_hdl,
										ret, p_app_value);
							} else {
								st_ble_servs_evt_data_t evt_data = { .conn_hdl =
										p_data->conn_hdl, .param_len =
										p_char->pp_descs[d]->app_size,
										.p_param = p_app_value, };
								p_serv->cb(
										BLE_SERVS_MULTI_ATTR_EVENT(
												p_char->pp_descs[d]->desc_idx,
	...
	case BLE_GATTS_EVENT_EXE_WRITE_RSP_COMP: {
		...
		if ((NULL != p_serv) && (NULL != p_char)) {
			if (BLE_GATTC_EXECUTE_WRITE_EXEC_FLAG
					== p_exe_write_evt_param->exe_flag) {
				/* Characteristic */
				if (((p_char->start_hdl + 1) == s_write_long_attr_hdl)
						&& (NULL != p_char->decode)) {
					ble_status_t ret;
					...
					if (NULL != p_char->pp_descs) {
						for (uint8_t d = 0; d < p_char->num_of_descs; d++) {
							if ((p_char->pp_descs[d]->attr_hdl
								...
								st_ble_gatt_value_t gatt_value;
								R_BLE_GATTS_GetAttr(p_data->conn_hdl,
										s_write_long_attr_hdl, &gatt_value);
								...
								if (NULL
									...
									p_char->pp_descs[d]->write_comp_cb(
											&p_char->pp_descs[d],
											p_data->conn_hdl, ret, p_app_value);
								} else {
...
void R_BLE_SERVS_VsCb(uint16_t type, ble_status_t result,
			...
			for (uint8_t s = 0; s < gs_num_of_servs; s++) {
				for (uint8_t c = 0; c < gs_servs[s]->num_of_chars; c++) {
					if (NULL != gs_servs[s]->pp_chars[c]->flow_ctrl_cb) {
						gs_servs[s]->pp_chars[c]->flow_ctrl_cb(
								gs_servs[s]->pp_chars[c]);
					}
				}
			}
		}
...
ble_status_t R_BLE_SERVS_SendHdlVal(const st_ble_servs_char_info_t *p_attr,
		uint16_t conn_hdl, const void *p_app_value, bool is_notify) {
	ble_status_t ret;

	if ((NULL == p_attr) || (NULL == p_attr->encode)
			|| (NULL == p_attr->pp_descs)) {
	...
	st_ble_gatt_hdl_value_pair_t hdl_val_data = { .attr_hdl = (uint16_t)(
			p_attr->start_hdl + 1), .value.value_len = p_attr->db_size,
	...
	if (BLE_SUCCESS == ret) {
		...
		} else if ((is_notify == false)
				&& (cccd & BLE_GATTS_CLI_CNFG_INDICATION)) {
...
ble_status_t R_BLE_SERVS_GetChar(const st_ble_servs_char_info_t *p_attr,
		uint16_t conn_hdl, void *p_app_value) {
	ble_status_t ret;
	...
	ret = R_BLE_GATTS_GetAttr(conn_hdl, (uint16_t)(p_attr->start_hdl + 1),
			&gatt_value);
...
ble_status_t R_BLE_SERVS_SetChar(const st_ble_servs_char_info_t *p_attr,
	...
	st_ble_gatt_value_t gatt_value = { .p_value = p_gatt_value, .value_len =
			p_attr->db_size, };

	ret = p_attr->encode(p_app_value, &gatt_value);

	if (BLE_SUCCESS == ret) {
		ret = R_BLE_GATTS_SetAttr(conn_hdl, (uint16_t)(p_attr->start_hdl + 1),
				&gatt_value);
	}
