...
static ble_status_t decode_st_ble_gapc_dev_name_t(
		st_ble_gapc_dev_name_t *p_app_value,
...
static ble_status_t encode_st_ble_gapc_dev_name_t(
		const st_ble_gapc_dev_name_t *p_app_value,
...
const st_ble_servc_char_info_t gs_dev_name_char = { .uuid_16 =
		BLE_GAPC_DEV_NAME_UUID, .uuid_type = BLE_GATT_16_BIT_UUID_FORMAT,
		.app_size = sizeof(st_ble_gapc_dev_name_t), .db_size =
				BLE_GAPC_DEV_NAME_LEN, .char_idx = BLE_GAPC_DEV_NAME_IDX,
		.p_attr_hdls = gs_dev_name_char_ranges, .decode =
				(ble_servc_attr_decode_t) decode_st_ble_gapc_dev_name_t,
		.encode = (ble_servc_attr_encode_t) encode_st_ble_gapc_dev_name_t, };

ble_status_t R_BLE_GAPC_WriteDevName(uint16_t conn_hdl,
		const st_ble_gapc_dev_name_t *p_value) // @suppress("API function naming")
		{
...
const st_ble_servc_char_info_t gs_appearance_char = { .uuid_16 =
		BLE_GAPC_APPEARANCE_UUID, .uuid_type = BLE_GATT_16_BIT_UUID_FORMAT,
		.app_size = sizeof(uint16_t), .db_size = sizeof(uint16_t), .char_idx =
				BLE_GAPC_APPEARANCE_IDX, .p_attr_hdls =
				gs_appearance_char_ranges, .decode =
				(ble_servc_attr_decode_t) decode_uint16_t, .encode =
				(ble_servc_attr_encode_t) encode_uint16_t, };
...
static ble_status_t decode_st_ble_gapc_per_pref_conn_param_t(
		st_ble_gapc_per_pref_conn_param_t *p_app_value,
		const st_ble_gatt_value_t *p_gatt_value) {
	uint32_t pos = 0;
...
static ble_status_t encode_st_ble_gapc_per_pref_conn_param_t(
		const st_ble_gapc_per_pref_conn_param_t *p_app_value,
		st_ble_gatt_value_t *p_gatt_value) {
	uint32_t pos = 0;
...
const st_ble_servc_char_info_t gs_per_pref_conn_param_char =
		{ .uuid_16 = BLE_GAPC_PER_PREF_CONN_PARAM_UUID, .uuid_type =
				BLE_GATT_16_BIT_UUID_FORMAT, .app_size =
				sizeof(st_ble_gapc_per_pref_conn_param_t), .db_size =
				BLE_GAPC_PER_PREF_CONN_PARAM_LEN, .char_idx =
				BLE_GAPC_PER_PREF_CONN_PARAM_IDX, .p_attr_hdls =
				gs_per_pref_conn_param_char_ranges,
				.decode =
						(ble_servc_attr_decode_t) decode_st_ble_gapc_per_pref_conn_param_t,
				.encode =
						(ble_servc_attr_encode_t) encode_st_ble_gapc_per_pref_conn_param_t, };
...
const st_ble_servc_char_info_t gs_cent_addr_rslv_char = { .uuid_16 =
		BLE_GAPC_CENT_ADDR_RSLV_UUID, .uuid_type = BLE_GATT_16_BIT_UUID_FORMAT,
		.app_size = sizeof(uint8_t), .db_size = sizeof(uint8_t), .char_idx =
				BLE_GAPC_CENT_ADDR_RSLV_IDX, .p_attr_hdls =
				gs_cent_addr_rslv_char_ranges, .decode =
				(ble_servc_attr_decode_t) decode_uint8_t, .encode =
				(ble_servc_attr_encode_t) encode_uint8_t, };
...
const st_ble_servc_char_info_t gs_rslv_priv_addr_only_char = { .uuid_16 =
		BLE_GAPC_RSLV_PRIV_ADDR_ONLY_UUID, .uuid_type =
		BLE_GATT_16_BIT_UUID_FORMAT, .app_size = sizeof(uint8_t), .db_size =
		sizeof(uint8_t), .char_idx = BLE_GAPC_RSLV_PRIV_ADDR_ONLY_IDX,
		.p_attr_hdls = gs_rslv_priv_addr_only_char_ranges, .decode =
				(ble_servc_attr_decode_t) decode_uint8_t, .encode =
				(ble_servc_attr_encode_t) encode_uint8_t, };
...
const st_ble_servc_char_info_t *gspp_gapc_chars[] = { &gs_dev_name_char,
		&gs_appearance_char, &gs_per_pref_conn_param_char,
		&gs_cent_addr_rslv_char, &gs_rslv_priv_addr_only_char, };

static st_ble_servc_info_t gs_client_info = { .pp_chars = gspp_gapc_chars,
		.num_of_chars = ARRAY_SIZE(gspp_gapc_chars), .p_attr_hdls =
				gs_gapc_ranges, };
...
void R_BLE_GAPC_ServDiscCb(uint16_t conn_hdl, uint8_t serv_idx, uint16_t type,
		void *p_param) // @suppress("API function naming")
		{
