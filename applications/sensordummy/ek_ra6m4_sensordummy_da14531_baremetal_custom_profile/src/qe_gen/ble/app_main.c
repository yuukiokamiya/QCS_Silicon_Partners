...
static void handle_read_dummy_data(uint16_t id, uint8_t const *const data);
...
/*End adding @MAIN_INITIALIZATION code */
	...
	while (1) {
		...
		if (true == notify_cfm) {
			...
			g_dummy_sensor0_read(&dummy_data);
...
static void handle_read_dummy_data(uint16_t id, uint8_t const *const data) {
	FSP_PARAMETER_NOT_USED(data);
	printf("sending id: 0x%04x\n", id);
	qc_svc_send_read_response(QC_SVC_SUCCESS, id, sizeof(dummy_data),
			(uint8_t*) &dummy_data);
}
