...
static bool r_ble_disc_next_inc_serv(uint16_t conn_hdl) {
	if (gs_inc_serv_pos < gs_num_of_inc_servs) {
		...
		for (uint8_t i = gs_inc_serv_pos; i < gs_num_of_inc_servs; i++) {
			for (uint8_t j = 0;
					j < gs_prim_entries[gs_prim_entry_pos].num_of_inc_servs;
					j++) {
				st_ble_disc_entry_t *p_inc_serv =
						&gs_prim_entries[gs_prim_entry_pos].inc_servs[j];

				if (p_inc_serv->uuid_type == gs_inc_servs[i].uuid_type) {
					if (((BLE_GATT_16_BIT_UUID_FORMAT == p_inc_serv->uuid_type)
							&& (((p_inc_serv->p_uuid[1] << 8)
									| (p_inc_serv->p_uuid[0]))
									== gs_inc_servs[i].value.inc_serv_16.service.uuid_16))
							|| ((BLE_GATT_128_BIT_UUID_FORMAT
									== p_inc_serv->uuid_type)
									&& (0
											== memcmp(p_inc_serv->p_uuid,
													gs_inc_servs[i].value.inc_serv_128.service.uuid_128,
													BLE_GATT_128_BIT_UUID_SIZE)))) {
						gs_serv_range.start_hdl =
								gs_inc_servs[i].value.inc_serv_16.service.range.start_hdl;
						gs_serv_range.end_hdl =
								gs_inc_servs[i].value.inc_serv_16.service.range.end_hdl;
...
static ble_status_t r_ble_start_serv_disc(uint16_t conn_hdl) {
	...
	if (BLE_SUCCESS != ret) {
		r_ble_stop_serv_disc(conn_hdl,
				(0 < gs_prim_entry_pos) ? (true) : (false));
	}
...
static void r_ble_start_desc_disc(uint16_t conn_hdl) {
	...
	if (gs_char_pos < (gs_num_of_chars - 1)) {
		if (BLE_GATT_16_BIT_UUID_FORMAT
				== gs_chars[gs_char_pos + 1].uuid_type) {
...
static void r_ble_disc_gattc_cb(uint16_t type, ble_status_t result,
		st_ble_gattc_evt_data_t *p_data) // @suppress("Function length")
		{
	...
	case BLE_GATTC_EVENT_PRIM_SERV_16_DISC_IND: {
		...
		for (uint8_t i = 0;
				i < (p_data->param_len / sizeof(st_ble_gattc_serv_16_evt_t));
				i++) {
			if (gs_num_of_servs == gs_prim_entries[gs_prim_entry_pos].idx) {
				st_disc_serv_param_t serv_param = { .uuid_type =
						BLE_GATT_16_BIT_UUID_FORMAT, .value.serv_16.range =
						p_serv_uuid_16_evt_params[i].range,
						.value.serv_16.uuid_16 =
								p_serv_uuid_16_evt_params[i].uuid_16, };

				gs_prim_entries[gs_prim_entry_pos].serv_cb(p_data->conn_hdl,
						gs_prim_entries[gs_prim_entry_pos].idx,
						BLE_DISC_PRIM_SERV_FOUND, &serv_param);

				memcpy(&gs_serv_range, &p_serv_uuid_16_evt_params[i].range,
						sizeof(gs_serv_range));
			}
	...
	case BLE_GATTC_EVENT_PRIM_SERV_128_DISC_IND: {
		...
		for (uint8_t i = 0;
				i < (p_data->param_len / sizeof(st_ble_gattc_serv_128_evt_t));
				i++) {
			if (gs_num_of_servs == gs_prim_entries[gs_prim_entry_pos].idx) {
				st_disc_serv_param_t serv_param = { .uuid_type =
						BLE_GATT_128_BIT_UUID_FORMAT, .value.serv_128.range =
						p_serv_uuid_128_evt_params[i].range, };
				...
				gs_prim_entries[gs_prim_entry_pos].serv_cb(p_data->conn_hdl,
						gs_prim_entries[gs_prim_entry_pos].idx,
						BLE_DISC_PRIM_SERV_FOUND, &serv_param);
				memcpy(&gs_serv_range, &p_serv_uuid_128_evt_params[i].range,
						sizeof(gs_serv_range));
			}
	...
	case BLE_GATTC_EVENT_PRIM_SERV_DISC_COMP: {
		if (0 == gs_num_of_servs) {
			gs_prim_entries[gs_prim_entry_pos].serv_cb(p_data->conn_hdl,
					gs_prim_entries[gs_prim_entry_pos].idx,
					BLE_DISC_PRIM_SERV_NOT_FOUND, NULL);
		}

		r_ble_all_disc_process(BLE_GATTC_EVENT_PRIM_SERV_DISC_COMP,
				p_data->conn_hdl);
	}
		break;
	...
	case BLE_GATTC_EVENT_INC_SERV_16_DISC_IND: {
		...
		for (uint8_t i = 0;
				i < (p_data->param_len / sizeof(st_ble_gattc_inc_serv_16_evt_t));
				i++) {
			gs_inc_servs[gs_num_of_inc_servs].uuid_type =
	...
	case BLE_GATTC_EVENT_INC_SERV_DISC_COMP: {
		r_ble_all_disc_process(BLE_GATTC_EVENT_INC_SERV_DISC_COMP,
				p_data->conn_hdl);
	}
		break;
	...
	case BLE_GATTC_EVENT_CHAR_16_DISC_IND: {
		...
		for (uint8_t i = 0;
				i < (p_data->param_len / sizeof(st_ble_gattc_char_16_evt_t));
				i++) {
	...
	case BLE_GATTC_EVENT_CHAR_128_DISC_IND: {
		...
		for (uint8_t i = 0;
				i < (p_data->param_len / sizeof(st_ble_gattc_char_128_evt_t));
				i++) {
	...
	case BLE_GATTC_EVENT_ALL_CHAR_DISC_COMP: {
		r_ble_all_disc_process(BLE_GATTC_EVENT_ALL_CHAR_DISC_COMP,
				p_data->conn_hdl);
	}
		break;
	...
	case BLE_GATTC_EVENT_CHAR_DESC_128_DISC_IND: {
		...
		for (uint8_t i = 0;
				...
				i++) {
			gs_descs[gs_num_of_descs].uuid_type = BLE_GATT_128_BIT_UUID_FORMAT;
	...
	case BLE_GATTC_EVENT_ALL_CHAR_DESC_DISC_COMP: {
		...
		if (0 == gs_state) {
			gs_prim_entries[gs_prim_entry_pos].serv_cb(p_data->conn_hdl,
					gs_prim_entries[gs_prim_entry_pos].idx, BLE_DISC_CHAR_FOUND,
			...
			gs_prim_entries[gs_prim_entry_pos].inc_servs[gs_target_inc_serv_pos].serv_cb(
					...
					BLE_DISC_CHAR_FOUND, &gs_chars[gs_char_pos]);
		} else {
			/* Do nothing. */
		}

		r_ble_all_disc_process(BLE_GATTC_EVENT_ALL_CHAR_DESC_DISC_COMP,
				p_data->conn_hdl);
	}
		break;
...
ble_status_t R_BLE_DISC_Start(uint16_t conn_hdl,
		const st_ble_disc_entry_t *p_entries, uint8_t num_of_entires,
