...
ble_status_t decode_16bit(uint16_t *p_app_value,
	...
	*p_app_value = (uint16_t)(
			(p_gatt_value->p_value[0]) | (p_gatt_value->p_value[1] << 8));
...
uint8_t pack_st_ble_ieee11073_sfloat_t(uint8_t *p_dst,
	...
	uint8_t pos = 0;
	uint16_t sfloat = (uint16_t)(
			(p_src->mantissa & 0x0FFF) | (p_src->exponent << 12));
