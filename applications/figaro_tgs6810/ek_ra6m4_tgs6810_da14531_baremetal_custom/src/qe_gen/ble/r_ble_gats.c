...
static const st_ble_servs_desc_info_t gs_serv_changed_cli_cnfg = { .attr_hdl =
		BLE_GATS_SERV_CHGED_CLI_CNFG_DESC_HDL, .app_size = sizeof(uint16_t),
		.desc_idx = BLE_GATS_SERV_CHGED_CLI_CNFG_IDX, .db_size =
				BLE_GATS_SERV_CHGED_CLI_CNFG_LEN, .decode =
				(ble_servs_attr_decode_t) decode_uint16_t, .encode =
				(ble_servs_attr_encode_t) encode_uint16_t, };

ble_status_t R_BLE_GATS_SetServChangedCliCnfg(uint16_t conn_hdl,
		const uint16_t *p_value) {
	return R_BLE_SERVS_SetDesc(&gs_serv_changed_cli_cnfg, conn_hdl,
			(const void*) p_value);
}

ble_status_t R_BLE_GATS_GetServChangedCliCnfg(uint16_t conn_hdl,
		uint16_t *p_value) {
	return R_BLE_SERVS_GetDesc(&gs_serv_changed_cli_cnfg, conn_hdl,
			(void*) p_value);
}
...
static ble_status_t decode_st_ble_gats_serv_changed_t(
		st_ble_gats_serv_changed_t *p_app_value,
...
static ble_status_t encode_st_ble_gats_serv_changed_t(
		const st_ble_gats_serv_changed_t *p_app_value,
...
static const st_ble_servs_char_info_t gs_serv_changed_char = { .start_hdl =
		BLE_GATS_SERV_CHGED_DECL_HDL, .end_hdl =
		BLE_GATS_SERV_CHGED_CLI_CNFG_DESC_HDL, .char_idx =
		BLE_GATS_SERV_CHGED_IDX, .app_size = sizeof(st_ble_gats_serv_changed_t),
		.db_size = BLE_GATS_SERV_CHGED_LEN, .decode =
				(ble_servs_attr_decode_t) decode_st_ble_gats_serv_changed_t,
		.encode = (ble_servs_attr_encode_t) encode_st_ble_gats_serv_changed_t,
		.pp_descs = gspp_serv_changed_descs, .num_of_descs = ARRAY_SIZE(
				gspp_serv_changed_descs), };

ble_status_t R_BLE_GATS_IndicateServChanged(uint16_t conn_hdl,
		const st_ble_gats_serv_changed_t *p_value) {
	return R_BLE_SERVS_SendHdlVal(&gs_serv_changed_char, conn_hdl,
			(const void*) p_value, false);
}
...
static const st_ble_servs_char_info_t *gspp_chars[] = { &gs_serv_changed_char, };
